...
void TProceso::ligaProceso(string name,int id){
	...
	cout<<"Ingrese el Tamaño del Proceso "<<nuevo->name<<"en MBs"<<endl; 
	nuevo->tam=condicion(x);
	if(nuevo->tam>MAX2){ 
	do{ 
		cout<<"El Tamaño del Proceso es demasiado grande para la memoria Actual"<<endl;  
		cout<<"Elija un tamaño más pequeño para el proceso "<<nuevo->name<<"en MBs"<<endl;
		...
		switch(op){
			case 1: switch(menu2()){
						...
						case 4: id=1+rand()%100;
								...
								system("pause");
								break;
						default:cout<<termcolor::red<<"Opción Inválida. Será retornado al menú principal"<<termcolor::reset<<endl;
								system("pause");
								break; 
					}
					break;
			...
			case 3: system("cls");
					...
					if(stat!=0){
					system("cls");
					cout<<termcolor::green<<"Ha finalizado el procesamiento con Éxito"<<termcolor::reset<<endl;
					system("pause");	
					}
					break;
